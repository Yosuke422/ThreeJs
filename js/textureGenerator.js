// Simple texture generator for the maze game
// This creates procedural textures that can be used in the game

export function generateTexture(type, size = 512) {
    const canvas = document.createElement('canvas');
    canvas.width = size;
    canvas.height = size;
    const ctx = canvas.getContext('2d');
    
    // Fill with background color
    ctx.fillStyle = '#000';
    ctx.fillRect(0, 0, size, size);
    
    switch (type) {
        case 'wood':
            generateWoodTexture(ctx, size);
            break;
        case 'stone':
            generateStoneTexture(ctx, size);
            break;
        case 'brick':
            generateBrickTexture(ctx, size);
            break;
        case 'sand':
            generateSandTexture(ctx, size);
            break;
        case 'metal':
            generateMetalTexture(ctx, size);
            break;
        case 'alien':
            generateAlienTexture(ctx, size);
            break;
        case 'ice':
            generateIceTexture(ctx, size);
            break;
        case 'lava':
            generateLavaTexture(ctx, size);
            break;
        case 'neon':
            generateNeonTexture(ctx, size);
            break;
        case 'coral':
            generateCoralTexture(ctx, size);
            break;
        default:
            generateDefaultTexture(ctx, size);
    }
    
    return canvas;
}

// Generate a wooden texture with grain
function generateWoodTexture(ctx, size) {
    // Base wood color
    ctx.fillStyle = '#8B4513';
    ctx.fillRect(0, 0, size, size);
    
    // Add wood grain
    for (let i = 0; i < size; i += 4) {
        const grainWidth = Math.random() * 2 + 1;
        const brightness = Math.random() * 20 - 10;
        
        ctx.strokeStyle = `rgba(139, 69, 19, ${0.1 + Math.random() * 0.1})`;
        ctx.lineWidth = grainWidth;
        
        ctx.beginPath();
        ctx.moveTo(0, i);
        
        // Wavy grain lines
        for (let x = 0; x < size; x += size/10) {
            const y = i + Math.sin(x * 0.01) * 5;
            ctx.lineTo(x, y);
        }
        
        ctx.lineTo(size, i);
        ctx.stroke();
    }
    
    // Add some dark knots
    for (let i = 0; i < 5; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const radius = Math.random() * 20 + 5;
        
        const gradient = ctx.createRadialGradient(x, y, 0, x, y, radius);
        gradient.addColorStop(0, 'rgba(50, 25, 0, 0.8)');
        gradient.addColorStop(1, 'rgba(139, 69, 19, 0)');
        
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.arc(x, y, radius, 0, Math.PI * 2);
        ctx.fill();
    }
}

// Generate stone texture
function generateStoneTexture(ctx, size) {
    // Base stone color
    ctx.fillStyle = '#888888';
    ctx.fillRect(0, 0, size, size);
    
    // Add some noise
    for (let x = 0; x < size; x++) {
        for (let y = 0; y < size; y++) {
            if (Math.random() > 0.5) {
                const value = Math.random() * 30 - 15;
                ctx.fillStyle = `rgba(128, 128, 128, ${Math.random() * 0.05})`;
                ctx.fillRect(x, y, 1, 1);
            }
        }
    }
    
    // Add cracks
    for (let i = 0; i < 10; i++) {
        const startX = Math.random() * size;
        const startY = Math.random() * size;
        
        ctx.strokeStyle = 'rgba(0, 0, 0, 0.2)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(startX, startY);
        
        let x = startX;
        let y = startY;
        const length = Math.random() * 100 + 50;
        
        for (let j = 0; j < length; j++) {
            x += Math.random() * 6 - 3;
            y += Math.random() * 6 - 3;
            ctx.lineTo(x, y);
        }
        
        ctx.stroke();
    }
}

// Generate brick texture
function generateBrickTexture(ctx, size) {
    // Base brick color
    ctx.fillStyle = '#8B4513';
    ctx.fillRect(0, 0, size, size);
    
    // Brick dimensions
    const brickHeight = size / 10;
    const brickWidth = size / 5;
    const mortarSize = size / 50;
    
    // Draw bricks
    for (let y = 0; y < size; y += brickHeight + mortarSize) {
        const offset = (Math.floor(y / (brickHeight + mortarSize)) % 2) * (brickWidth / 2);
        
        for (let x = -brickWidth / 2; x < size; x += brickWidth + mortarSize) {
            // Vary brick colors slightly
            const r = 139 + Math.random() * 30 - 15;
            const g = 69 + Math.random() * 20 - 10;
            const b = 19 + Math.random() * 10 - 5;
            
            ctx.fillStyle = `rgb(${r}, ${g}, ${b})`;
            ctx.fillRect(x + offset, y, brickWidth, brickHeight);
            
            // Add texture to bricks
            ctx.fillStyle = 'rgba(0, 0, 0, 0.1)';
            for (let i = 0; i < 20; i++) {
                const spotX = x + offset + Math.random() * brickWidth;
                const spotY = y + Math.random() * brickHeight;
                const spotSize = Math.random() * 4 + 1;
                ctx.beginPath();
                ctx.arc(spotX, spotY, spotSize, 0, Math.PI * 2);
                ctx.fill();
            }
        }
    }
    
    // Draw mortar
    ctx.fillStyle = '#CCCCCC';
    
    // Horizontal mortar
    for (let y = brickHeight; y < size; y += brickHeight + mortarSize) {
        ctx.fillRect(0, y, size, mortarSize);
    }
    
    // Vertical mortar
    for (let row = 0; row < Math.ceil(size / (brickHeight + mortarSize)); row++) {
        const offset = (row % 2) * (brickWidth / 2);
        
        for (let x = offset; x < size; x += brickWidth + mortarSize) {
            ctx.fillRect(x - mortarSize / 2, row * (brickHeight + mortarSize), mortarSize, brickHeight);
        }
    }
}

// Generate sandy texture
function generateSandTexture(ctx, size) {
    // Base sand color
    ctx.fillStyle = '#D2B48C';
    ctx.fillRect(0, 0, size, size);
    
    // Add sand grains
    for (let i = 0; i < size * size / 10; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const grainSize = Math.random() * 2 + 0.5;
        
        const brightness = Math.random() * 40 - 20;
        ctx.fillStyle = `rgba(210, 180, 140, ${Math.random() * 0.2})`;
        
        ctx.beginPath();
        ctx.arc(x, y, grainSize, 0, Math.PI * 2);
        ctx.fill();
    }
    
    // Add some darker spots
    for (let i = 0; i < 200; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const spotSize = Math.random() * 4 + 1;
        
        ctx.fillStyle = 'rgba(160, 120, 80, 0.2)';
        ctx.beginPath();
        ctx.arc(x, y, spotSize, 0, Math.PI * 2);
        ctx.fill();
    }
}

// Generate metallic texture
function generateMetalTexture(ctx, size) {
    // Base metal color
    ctx.fillStyle = '#AAAAAA';
    ctx.fillRect(0, 0, size, size);
    
    // Add brushed metal effect
    for (let i = 0; i < size; i += 2) {
        const brightness = Math.random() * 30 - 15;
        ctx.strokeStyle = `rgba(170, 170, 170, ${0.05 + Math.random() * 0.05})`;
        ctx.lineWidth = 1;
        
        ctx.beginPath();
        ctx.moveTo(0, i);
        ctx.lineTo(size, i);
        ctx.stroke();
    }
    
    // Add some scratches
    for (let i = 0; i < 30; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const length = Math.random() * 40 + 10;
        const angle = Math.random() * Math.PI;
        
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.2)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(x, y);
        ctx.lineTo(x + Math.cos(angle) * length, y + Math.sin(angle) * length);
        ctx.stroke();
    }
}

// Generate alien texture
function generateAlienTexture(ctx, size) {
    // Base color
    ctx.fillStyle = '#1A472A';
    ctx.fillRect(0, 0, size, size);
    
    // Add veins and patterns
    for (let i = 0; i < 20; i++) {
        const startX = Math.random() * size;
        const startY = Math.random() * size;
        
        ctx.strokeStyle = 'rgba(0, 255, 0, 0.2)';
        ctx.lineWidth = Math.random() * 3 + 1;
        ctx.beginPath();
        ctx.moveTo(startX, startY);
        
        let x = startX;
        let y = startY;
        const length = Math.random() * 100 + 50;
        
        for (let j = 0; j < length; j++) {
            x += Math.random() * 10 - 5;
            y += Math.random() * 10 - 5;
            ctx.lineTo(x, y);
        }
        
        ctx.stroke();
    }
    
    // Add glowing spots
    for (let i = 0; i < 30; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const spotSize = Math.random() * 10 + 5;
        
        const gradient = ctx.createRadialGradient(x, y, 0, x, y, spotSize);
        gradient.addColorStop(0, 'rgba(0, 255, 100, 0.8)');
        gradient.addColorStop(1, 'rgba(26, 71, 42, 0)');
        
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.arc(x, y, spotSize, 0, Math.PI * 2);
        ctx.fill();
    }
}

// Generate ice texture
function generateIceTexture(ctx, size) {
    // Create ice background gradient
    const gradient = ctx.createLinearGradient(0, 0, size, size);
    gradient.addColorStop(0, '#AADDFF');
    gradient.addColorStop(1, '#88CCFF');
    ctx.fillStyle = gradient;
    ctx.fillRect(0, 0, size, size);
    
    // Add cracks
    for (let i = 0; i < 15; i++) {
        const startX = Math.random() * size;
        const startY = Math.random() * size;
        
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.4)';
        ctx.lineWidth = Math.random() * 2 + 0.5;
        ctx.beginPath();
        ctx.moveTo(startX, startY);
        
        let x = startX;
        let y = startY;
        let branches = Math.floor(Math.random() * 5) + 3;
        
        // Create branching cracks
        createIceCrack(ctx, x, y, 50, 0, 3);
    }
    
    // Add some highlights
    for (let i = 0; i < 50; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const spotSize = Math.random() * 5 + 1;
        
        ctx.fillStyle = 'rgba(255, 255, 255, 0.3)';
        ctx.beginPath();
        ctx.arc(x, y, spotSize, 0, Math.PI * 2);
        ctx.fill();
    }
}

// Recursive function to create branching ice cracks
function createIceCrack(ctx, x, y, length, angle, depth) {
    if (depth <= 0) return;
    
    const endX = x + Math.cos(angle) * length;
    const endY = y + Math.sin(angle) * length;
    
    ctx.beginPath();
    ctx.moveTo(x, y);
    
    // Create a slightly jagged line
    const segments = Math.floor(length / 10) + 1;
    let currentX = x;
    let currentY = y;
    
    for (let i = 1; i <= segments; i++) {
        const segX = x + (endX - x) * (i / segments) + (Math.random() * 4 - 2);
        const segY = y + (endY - y) * (i / segments) + (Math.random() * 4 - 2);
        ctx.lineTo(segX, segY);
        currentX = segX;
        currentY = segY;
    }
    
    ctx.stroke();
    
    // Create branches
    if (depth > 1 && Math.random() > 0.3) {
        const branchAngle1 = angle + Math.random() * Math.PI/4 + Math.PI/8;
        const branchAngle2 = angle - Math.random() * Math.PI/4 - Math.PI/8;
        const branchLength = length * (0.4 + Math.random() * 0.3);
        
        createIceCrack(ctx, currentX, currentY, branchLength, branchAngle1, depth - 1);
        createIceCrack(ctx, currentX, currentY, branchLength, branchAngle2, depth - 1);
    }
}

// Generate lava texture
function generateLavaTexture(ctx, size) {
    // Base color
    ctx.fillStyle = '#800000';
    ctx.fillRect(0, 0, size, size);
    
    // Create lava blobs
    for (let i = 0; i < 40; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const blobSize = Math.random() * 50 + 20;
        
        const gradient = ctx.createRadialGradient(x, y, 0, x, y, blobSize);
        gradient.addColorStop(0, 'rgba(255, 100, 0, 0.8)');
        gradient.addColorStop(0.7, 'rgba(200, 0, 0, 0.5)');
        gradient.addColorStop(1, 'rgba(128, 0, 0, 0)');
        
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.arc(x, y, blobSize, 0, Math.PI * 2);
        ctx.fill();
    }
    
    // Add cracks
    for (let i = 0; i < 30; i++) {
        const startX = Math.random() * size;
        const startY = Math.random() * size;
        
        ctx.strokeStyle = 'rgba(0, 0, 0, 0.3)';
        ctx.lineWidth = Math.random() * 3 + 1;
        ctx.beginPath();
        ctx.moveTo(startX, startY);
        
        let x = startX;
        let y = startY;
        const length = Math.random() * 50 + 20;
        
        for (let j = 0; j < length; j++) {
            x += Math.random() * 6 - 3;
            y += Math.random() * 6 - 3;
            ctx.lineTo(x, y);
        }
        
        ctx.stroke();
    }
}

// Generate neon texture
function generateNeonTexture(ctx, size) {
    // Black base
    ctx.fillStyle = '#000000';
    ctx.fillRect(0, 0, size, size);
    
    // Create grid pattern
    const gridSize = size / 8;
    
    // Main grid lines
    ctx.strokeStyle = 'rgba(0, 200, 255, 0.7)';
    ctx.lineWidth = 2;
    
    // Horizontal lines
    for (let y = 0; y <= size; y += gridSize) {
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(size, y);
        ctx.stroke();
    }
    
    // Vertical lines
    for (let x = 0; x <= size; x += gridSize) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, size);
        ctx.stroke();
    }
    
    // Add glow
    ctx.strokeStyle = 'rgba(0, 200, 255, 0.3)';
    ctx.lineWidth = 4;
    
    // Horizontal glow
    for (let y = 0; y <= size; y += gridSize) {
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(size, y);
        ctx.stroke();
    }
    
    // Vertical glow
    for (let x = 0; x <= size; x += gridSize) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, size);
        ctx.stroke();
    }
    
    // Add some brighter intersections
    for (let x = 0; x <= size; x += gridSize) {
        for (let y = 0; y <= size; y += gridSize) {
            ctx.fillStyle = 'rgba(0, 200, 255, 0.7)';
            ctx.beginPath();
            ctx.arc(x, y, 3, 0, Math.PI * 2);
            ctx.fill();
            
            // Add glow
            ctx.fillStyle = 'rgba(0, 200, 255, 0.2)';
            ctx.beginPath();
            ctx.arc(x, y, 6, 0, Math.PI * 2);
            ctx.fill();
        }
    }
}

// Generate coral texture
function generateCoralTexture(ctx, size) {
    // Base color
    ctx.fillStyle = '#FF6F61';
    ctx.fillRect(0, 0, size, size);
    
    // Add texture
    for (let i = 0; i < 100; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const radius = Math.random() * 30 + 5;
        
        // Create circular coral formations
        const gradient = ctx.createRadialGradient(x, y, 0, x, y, radius);
        
        // Vary the coral colors
        const hue = 5 + Math.random() * 20; // reddish hues
        gradient.addColorStop(0, `hsla(${hue}, 80%, 70%, 0.7)`);
        gradient.addColorStop(0.7, `hsla(${hue}, 70%, 60%, 0.4)`);
        gradient.addColorStop(1, 'rgba(255, 111, 97, 0)');
        
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.arc(x, y, radius, 0, Math.PI * 2);
        ctx.fill();
    }
    
    // Add small holes pattern
    for (let i = 0; i < 500; i++) {
        const x = Math.random() * size;
        const y = Math.random() * size;
        const holeSize = Math.random() * 3 + 1;
        
        ctx.fillStyle = 'rgba(150, 50, 50, 0.4)';
        ctx.beginPath();
        ctx.arc(x, y, holeSize, 0, Math.PI * 2);
        ctx.fill();
    }
}

// Generate default texture with a subtle pattern
function generateDefaultTexture(ctx, size) {
    // Base color
    ctx.fillStyle = '#888888';
    ctx.fillRect(0, 0, size, size);
    
    // Add some noise
    for (let x = 0; x < size; x += 2) {
        for (let y = 0; y < size; y += 2) {
            if (Math.random() > 0.5) {
                const value = Math.random() * 20 - 10;
                ctx.fillStyle = `rgba(128, 128, 128, ${Math.random() * 0.1})`;
                ctx.fillRect(x, y, 2, 2);
            }
        }
    }
} 